...
our $VERSION = '0.518';
use Exporter 'import';
our @EXPORT_OK = qw( fill_form settings_menu read_line );
...
sub settings_menu { # ###
    if ( ref $_[0] ne 'Term::Form' ) {
        return settings_menu( bless( { %{ _defaults() }, plugin => $Plugin->new() }, 'Term::Form' ), @_ );
    }
    my ( $self, $menu, $curr, $opt ) = @_;
    croak "'settings_menu' called with no argument."                            if ! defined $menu;
    croak "'settings_menu' requires an ARRAY reference as its first argument."  if ref $menu ne 'ARRAY';
    croak "'settings_menu' called with one argument. Two arguents are required" if ! defined $curr;
    croak "'settings_menu' requires a HASH reference as its second argument."   if ref $curr ne 'HASH';
    if ( ! defined $opt ) {
        $opt = {};
    }
    croak "'settings_menu': the optional third argument must be a HASH reference" if ref $opt ne 'HASH';
    if ( %$opt ) {
        validate_options( _valid_options( 'settings_menu' ), $opt );
        for my $key ( keys %$opt ) {
            $self->{$key} = $opt->{$key} if defined $opt->{$key};
        }
    }
    ...
    for my $sub ( @$menu ) {
        ...
        push @$list, [ $name, '[' . $current . ']' ]; # _sanitized_string( $_->[0] )
    }
    ...
    CHAR: while ( 1 ) {
        ...
        if ( $self->{i}{beep} ) {
            ...
            if ( $self->{i}{curr_row} < @{$self->{i}{pre}} ) {
                ...
                my $key = unicode_sprintf( $list->[$self->{i}{curr_row}][0], $self->{i}{max_key_w}, 0 );
                print $key . $self->{i}{sep} . $list->[$self->{i}{curr_row}][1];
            }
            print RESET;
        }
        my $char = $self->{plugin}->__get_key_OS();
        ...
        if ( $char == VK_RIGHT ) {
            $k = 1;
            $self->__value_to_right( $list, $menu, $new );
        }
        elsif ( $char == VK_LEFT ) {
            $k = 1;
            $self->__value_to_left( $list, $menu, $new );
        }
        elsif ( $char == VK_END   || $char == CONTROL_E ) {
            $k = 1;
        #    $self->__end( $m );
...
sub __value_to_right { # ###
    my ( $self, $list, $menu, $new ) = @_;
    if ( $self->{i}{curr_row} <= $#{$self->{i}{pre}} ) {
        $self->{i}{beep} = 1;
    }
    else {
        my ( $key, $name, $values ) = @{$menu->[ $self->{i}{curr_row} - @{$self->{i}{pre}} ]};
        $new->{$key}++;
        if ( $new->{$key} == @$values ) {
            $new->{$key} = 0;
        }
        $list->[$self->{i}{curr_row}][1] = '[' . $values->[$new->{$key}] . ']';
    }
}
